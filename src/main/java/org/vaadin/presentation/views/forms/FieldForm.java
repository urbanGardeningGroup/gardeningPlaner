 ...
 */
@Dependent
public class FieldForm extends AbstractForm<Field> {
    ...
    TextField fieldName = new MTextField("Name").withFullWidth();
     ...
     */
    @Inject
    @FieldEvent(FieldEvent.Type.SAVE)
    javax.enterprise.event.Event<Field> saveEvent;
    @Inject
    @FieldEvent(FieldEvent.Type.REFRESH)
    javax.enterprise.event.Event<Field> refrehsEvent;
    @Inject
    @FieldEvent(FieldEvent.Type.DELETE)
    javax.enterprise.event.Event<Field> deleteEvent;
    ...
    protected Component createContent() {
        ...
        return new MVerticalLayout(
                new Header("Edit field").setHeaderLevel(3),
                new MFormLayout(
                        fieldName
                ).withFullWidth(),
                getToolbar()
    ...
    void init() {
        ...
        setSavedHandler(new SavedHandler<Field>() {

            @Override
            public void onSave(Field entity) {
                try {
                    ...
                    /*
                     * The field object uses optimitic locking with the
                     * version field. Notify user the editing didn't succeed.
                     */
                    Notification.show("The field was concurrently edited "
        ...
        setResetHandler(new ResetHandler<Field>() {

            @Override
            public void onReset(Field entity) {
                refrehsEvent.fire(entity);
            }
        });
        setDeleteHandler(new DeleteHandler<Field>() {
            @Override
            public void onDelete(Field entity) {
